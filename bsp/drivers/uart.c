...
typedef struct
{
	uart_callback_t tx_done;
	uart_callback_t rx_done;
} uart_callbacks_t;
...
{
	/* ESTA FUNCIÓN SE DEFINIRÁ EN LAS PRÁCTICAS 8 y 9 */
...
/**
 * Vacía el búfer circular de transmisión de la uart hasta que su cola HW no admite más
 * caracteres
 * @param uart	Identificador de la uart
 */
static inline void flush_tx_buffer (uart_id_t uart)
{
	/* ESTA FUNCIÓN SE DEFINIRÁ EN LA PRÁCTICA 8 */
}
...
/**
 * Rellena el búfer circular de recepción de la uart hasta que su cola HW no tenga más
 * caracteres
 * @param uart	Identificador de la uart
 */
static inline void fill_rx_buffer (uart_id_t uart)
{
	/* ESTA FUNCIÓN SE DEFINIRÁ EN LA PRÁCTICA 8 */
}
 ...
 * @param count	Número de bytes recibidos
 * @return		El número de bytes almacenados en el búfer
 */
